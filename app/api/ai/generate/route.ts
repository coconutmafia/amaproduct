...
${contentType !== 'post' ? `⚠️ ГОЛОС ВО ВСЕХ ТЕКСТОВЫХ ПОЛЯХ JSON (hook, headline, body, description_text, voiceover, speech, subject и т.д.): живой язык этого блогера. БЕЗ тире «—». Без телеграфных обрывков («Не А. Не Б. А В.»). Без канцелярита и пустых обещаний («скажу конкретно», «разбираю внутри») — только конкретика. Эти поля читает подписчик, правила как для постов.` : ''}
