...
ФОРМАТ ОТВЕТА
═══════════════════════════════
Ответь ТОЛЬКО валидным JSON без markdown-обёрток, без пояснений, без комментариев.
...

{
  ...
  "phases": [
    ...
    // ── SSE stream — sends keep-alive events so Vercel never times out ─────────
    ...
    const readable = new ReadableStream({
      async start(controller) {
        ...
        try {
          // Send immediately so the connection is alive before Claude starts
          ...
          for await (const chunk of claudeStream) {
            if (
              ...
              chunkCount++
              // Send a keep-alive ping every 50 chunks so Vercel edge stays alive
              if (chunkCount % 50 === 0) {
