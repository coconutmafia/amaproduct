...
export async function POST(request: Request) {
  try {
    ...
    if (blogLinesText) {
      const lineMatches = blogLinesText.match(/ЛИЧНАЯ ЛИНИЯ \d+\s*[—-]\s*([^\n]+)/g) || []
      for (const m of lineMatches) {
        const name = m.replace(/ЛИЧНАЯ ЛИНИЯ \d+\s*[—-]\s*/, '').trim()
        if (name && name !== 'Без названия') personalLineNames.push(name)
        else personalLineNames.push(`Личная линия ${personalLineNames.length + 1}`)
      }
      // If no named lines found but there's blog content, add a generic one
      if (personalLineNames.length === 0 && blogLinesText.includes('ЛИЧНАЯ ЛИНИЯ')) {
        personalLineNames.push('Личная линия 1')
      }
    }
...
${blogLinesText && personalLineNames.length > 0 ? (() => {
