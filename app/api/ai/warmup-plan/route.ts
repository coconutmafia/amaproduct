    ...
    const readable = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // Стримим ответ Claude чтобы отправлять keep-alive пинги
          ...
          for await (const chunk of claudeStream) {
            chunkCount++
            // Пинг каждые 30 чанков — браузер не закроет соединение
            if (chunkCount % 30 === 0) {
              send({ type: 'progress', chunks: chunkCount })
            }
          }

          // Получаем финальный результат с tool_use блоком
          ...
          // Отправляем пустой пинг после done — гарантирует что браузер получит последний чанк
          send({ type: 'end' })
          controller.close()
        } catch (err) {
