...
| Прямые эфиры | 1–2 в фазах 3–4 |`
    ...
    // Parse Anthropic SDK errors — they embed JSON like: {"type":"error","error":{"type":"...","message":"..."}}
    let humanMsg = 'AI недоступен'
    if (error instanceof Error) {
      const raw = error.message
      // Try to extract the nested Anthropic error message
      try {
        const jsonStart = raw.indexOf('{')
        if (jsonStart !== -1) {
          const parsed = JSON.parse(raw.slice(jsonStart)) as { error?: { message?: string }; message?: string }
          const inner = parsed?.error?.message || parsed?.message || ''
          if (inner) {
            if (inner.includes('credit') || inner.includes('balance')) {
              humanMsg = 'Закончились кредиты Anthropic. Пополните баланс на console.anthropic.com'
            } else if (inner.includes('credential') || inner.includes('API key') || inner.includes('auth')) {
              humanMsg = 'Неверный API ключ Anthropic. Проверьте ANTHROPIC_API_KEY в настройках'
            } else {
              humanMsg = inner
            }
          }
