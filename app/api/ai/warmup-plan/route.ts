    ...
    const readable = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // Гарантированный пинг каждые 5 сек — настоящее SSE-событие (не комментарий)
          // чтобы Vercel-прокси не буферизовал мелкие пакеты и мобильный браузер не закрыл соединение
          let pingCount = 0
          const pingInterval = setInterval(() => {
            try {
              pingCount++
              controller.enqueue(encoder.encode(`data: {"type":"ping","n":${pingCount}}\n\n`))
