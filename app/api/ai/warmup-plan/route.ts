...
Заполни все фазы полностью — каждый день от 1 до ${duration} должен быть в daily_plan соответствующей фазы.`
    ...
    // ── SSE + Tool Use: keep-alive пинги + гарантированный JSON ─────────────
    ...
    const readable = new ReadableStream({
      async start(controller) {
        ...
        // padding для первого сообщения — Vercel буферизует малые пакеты
        const sendFlushed = (data: Record<string, unknown>) => {
          const pad = ' '.repeat(900)
          controller.enqueue(encoder.encode(`data: ${JSON.stringify({ ...data, _pad: pad })}\n\n`))
        }

        try {
          sendFlushed({ type: 'status', message: 'Составляю план прогрева...' })
          ...
          // Пинг каждые 2 сек — настоящее SSE data-событие (не комментарий)
          // + padding до ~1KB чтобы Vercel/nginx принудительно сбрасывал TCP-буфер
          // без этого мобильные браузеры видят тишину и закрывают соединение
          const PAD = ' '.repeat(900) // итого ~1KB на пакет — гарантированный flush
          let pingCount = 0
          const pingInterval = setInterval(() => {
            try {
              pingCount++
              controller.enqueue(encoder.encode(`data: {"type":"ping","n":${pingCount},"_pad":"${PAD}"}\n\n`))
          ...
          try {
            ...
            for await (const _chunk of claudeStream) {
              // Читаем поток чтобы не блокировать — пинги идут через setInterval
            }
