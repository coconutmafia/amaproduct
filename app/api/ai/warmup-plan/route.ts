...
export async function POST(request: Request) {
  try {
    ...
    if (blogLinesText) {
      // Match "ЛИЧНАЯ ЛИНИЯ N — Name" or "ЛИЧНАЯ ЛИНИЯ N: Name" or just "ЛИЧНАЯ ЛИНИЯ N"
      const lineMatches = blogLinesText.match(/ЛИЧНАЯ ЛИНИЯ\s*\d*\s*[—:\-]?\s*([^\n]*)/gi) || []
      for (const m of lineMatches) {
        const name = m.replace(/ЛИЧНАЯ ЛИНИЯ\s*\d*\s*[—:\-]?\s*/i, '').trim()
