...
Заполни все фазы полностью — каждый день от 1 до ${duration} должен быть в daily_plan соответствующей фазы.`
    ...
    // ── Tool Use — гарантированно возвращает валидный JSON нужной схемы ─────
      ...
      tools: [
        {
          ...
          input_schema: {
            ...
            properties: {
              strategy_summary: {
                type: 'string',
                description: '2-3 предложения об общей стратегии для этого блогера и продукта',
              },
              phases: {
                type: 'array',
                items: {
                  type: 'object',
                  properties: {
                    phase: { type: 'string', enum: ['niche', 'expert', 'product', 'objections'] },
                    label: { type: 'string' },
                    ...
                    daily_plan: {
                      ...
                      items: {
                        ...
                        properties: {
                          day: { type: 'number' },
                          meaning: { type: 'string', description: 'Конкретный смысл дня — о чём говорить' },
                        },
    ...
    const planData = toolBlock.input
