...
Верни ПОЛНЫЙ обновлённый список кадров через инструмент edit_stories (по одному элементу на каждый кадр, в том же порядке).`
    ...
    const out = raw
      ...
      // Пустой элемент НА МЕСТЕ ручного кадра (у него headline/body пустые —
      // дизайн живёт картинкой) — ЗАКОННЫЙ: выбрасывать его нельзя, иначе
      // индексы съезжают и «добавленный» кадр съедается (виновник «добавь
      // пятый — вернулось четыре» после починки потолка). Пустые НОВЫЕ хвосты
      // (за пределами присланного списка) по-прежнему отбрасываем.
      .filter((r, i) => (r.headline || r.body) || (i < frames.length && !(frames[i]?.headline || frames[i]?.body)))
    if (out.length === 0 || out.every((r) => !(r.headline || r.body))) return NextResponse.json({ error: 'Не удалось применить правку — попробуй сформулировать иначе' }, { status: 502 })
