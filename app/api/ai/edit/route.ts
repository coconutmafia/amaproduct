...
- Если это структурный контент (карусель / рилз / сторис) — разбивай на понятные блоки обычным текстом, каждый с новой строки и пустой строкой между блоками. Например: ${(() => {
  const l = resolveContentLanguage(project)
  if (l === 'en') return '«Slide 1:» затем заголовок и текст слайда; «Scene 1 (0-3 sec):» затем что на экране и озвучка — метки блоков на английском, как и весь контент'
  if (l === 'es') return '«Diapositiva 1:» затем заголовок и текст слайда; «Escena 1 (0-3 seg):» затем что на экране и озвучка — метки блоков на испанском, как и весь контент'
  if (l === 'it') return '«Slide 1:» затем заголовок и текст слайда; «Scena 1 (0-3 sec):» затем что на экране и озвучка — метки блоков на итальянском, как и весь контент'
  if (l === 'de') return '«Slide 1:» затем заголовок и текст слайда; «Szene 1 (0-3 Sek):» затем что на экране и озвучка — метки блоков на немецком, как и весь контент'
  return '«Слайд 1:» затем заголовок и текст слайда; «Сцена 1 (0-3 сек):» затем что на экране и озвучка'
})()}. Читаемо, как для человека, а не как данные.`
