...
- ЯЗЫК: кадры на языке исходного текста (английский текст → английские кадры). Ничего не переводи.
...
Верни кадры через инструмент plan_stories (ориентир ~${target} кадров; если в тексте маркеры «Сторис N» — ровно по ним).`
    ...
    // RU + EN: у англоязычных блогов модель придумывала бы английские CTA мимо
    // кириллического фильтра («DM me», «link in bio») — ловим оба языка.
    const CTA_BLACKLIST = /(пиши|напиши|ответь|отвечай)[^.!?\n]{0,25}(директ|комментар|мне)|листай дальше|смотри до конца|ссылка в шапке|жми[^.!?\n]{0,20}(❤|🤍|сердеч|огон|плюс|\+)|dm me|link in (my )?bio|swipe up|keep (swiping|watching)|watch (till|until) the end|comment below|drop a [^.!?\n]{0,15}(below|in the comments)|tap the (heart|like)/i
