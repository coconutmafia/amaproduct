...
// Метки и порядок — 1-в-1 из эталонного листа урока («Касдевы Ава.xlsx»):
// Боли → Возражения → Хотелки/потребности → Триггеры (+ Преимущества из
// текста урока — в файле Авы секция не заполнена).
...
const MEANINGS_TYPE_ORDER: MeaningsRow['type'][] = ['pain', 'objection', 'need', 'trigger', 'advantage']
...
1. Категория (поле type):
   ...
   - need (хотелки/потребности): «с каким запросом пришёл», «а что ты хочешь, как хочешь, чтобы было» — точка Б. Этот вопрос звучит в КАЖДОМ интервью — потребности должны быть выписаны у каждого участника
