...
// Instagram scraping now lives in lib/scrape/instagramProfile.ts (Apify-first),
// shared with onboarding autofill so the owner's own profile reliably persists.
...
export async function POST(request: Request) {
  try {
    ...
    // The wizard passes a raw URL/handle; normalize to a bare handle before
    // scraping (the old code fed the full URL straight into the scrapers, which
    // built malformed t.me/instagram.com/… URLs and silently returned nothing).
    const handle = extractUsername(username)

    if (platform === 'telegram') {
      const posts = await scrapeTelegram(handle, 80)
      if (posts.length === 0) return NextResponse.json({ error: 'Постов не найдено. Убедись что канал публичный.' }, { status: 422 })
      postsCount = posts.length
      contentText = `Telegram-канал @${handle}\n\nПоследние ${postsCount} постов:\n\n` + posts.map((p, i) => `--- Пост ${i + 1} ---\n${p}`).join('\n\n')

    } else if (platform === 'instagram') {
      const { bio, posts } = await scrapeInstagramProfile(handle, 80)
      if (!bio && posts.length === 0) {
        return NextResponse.json({ error: 'Instagram заблокировал загрузку или профиль закрыт. Скопируй тексты постов и загрузи через «Материалы» вручную.' }, { status: 422 })
      }
      postsCount = posts.length + (bio ? 1 : 0)
      contentText = `Instagram @${handle}\n\n`
      if (bio) contentText += `Описание профиля:\n${bio}\n\n`
      if (posts.length > 0) contentText += `Последние ${posts.length} постов:\n\n` + posts.map((p, i) => `--- Пост ${i + 1} ---\n${p}`).join('\n\n')

    } else if (platform === 'youtube') {
      const { description, videos } = await scrapeYoutube(handle, 50)
      postsCount = videos.length + (description ? 1 : 0)
      contentText = `YouTube канал: ${handle}\n\n`
      ...
      const posts = await scrapeVK(handle, 50)
      postsCount = posts.length
      contentText = `VK страница: ${handle}\n\nПосты:\n\n` + posts.map((p, i) => `--- Пост ${i + 1} ---\n${p}`).join('\n\n')
    }

    if (!contentText) return NextResponse.json({ error: 'Не удалось загрузить контент' }, { status: 422 })

    // Save under a material_type that the generation context (ALWAYS_INCLUDE in
    // lib/ai/rag.ts) actually reads. 'other' was NOT in that list, so every
    // scraped social profile — including the owner's own Instagram — silently
    // never reached generation. The owner's Instagram is their own voice →
    // 'my_instagram'; Telegram/YouTube/VK have no dedicated slot → 'additional'
    // (also in ALWAYS_INCLUDE). Competitors go through a different flow.
    const materialTypeByPlatform: Record<string, string> = {
      instagram: 'my_instagram',
      telegram: 'additional',
      youtube: 'additional',
      vk: 'additional',
    }
    const targetType = materialTypeByPlatform[platform] ?? 'additional'
    const title = `${label} @${handle}`

    // Replace only THIS platform's prior scrape. Anchor on the `${label} @`
    // prefix (titles are always `${label} @username`) so 'VK' no longer matches
    // any title merely containing "vk", and so a user's own 'additional'
    // uploads are never deleted.
    await supabase.from('project_materials').delete()
      .eq('project_id', projectId).eq('material_type', targetType)
      .ilike('title', `${label} @%`)
