...
export function buildSystemPrompt(context: RAGContext, project: Project): string {
  ...
  // Залетевшие рилзы, которые блогер загрузил в «Тренды». Их видят план
  // прогрева и брифы; сюда добавлены после жалобы 26.08 («загрузила рилзы —
  // ассистент их не видит»), чтобы чат и AI-правка контент-плана тоже могли
  // на них опираться и перечислять их по просьбе.
  const reels = context.viralReels ?? []
  const reelsSection = reels.length > 0 ? `
═══════════════════════════════════════
ЗАЛЕТЕВШИЕ РИЛЗЫ, КОТОРЫЕ ЗАГРУЗИЛ САМ БЛОГЕР (${reels.length} шт.)
═══════════════════════════════════════
Это референсы, которые он сам отобрал и загрузил в разделе «Тренды».
Можешь их перечислять, выбирать подходящие под тему дня и предлагать
адаптацию под его нишу и голос. Ссылайся на них по НОМЕРУ и формату.
${reels.map((r, i) => `${i + 1}. Формат: ${r.reel_type}${r.username ? ` · автор @${r.username}` : ''}${r.views ? ` · ${r.views} просмотров` : ''}
   Разбор: ${r.analysis || '—'}`).join('\n')}
` : ''
