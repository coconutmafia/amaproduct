// Ядро «Плана недели» (недельные брифы контент-плана): RAG-контекст, тренды,
// психология фаз, форс-тул и один прогон Claude. Вынесено из
// app/api/ai/generate-week-brief 24.08, чтобы фоновый джоб
// (lib/jobs/runWeekBriefJob.ts) использовал РОВНО ту же логику, что и
// sync-роут — промпт не раздваивается (паттерн lib/ai/warmupPlan.ts).
...
export interface WeekBriefDayResult {
  day: number
  brief: Record<string, string>
}

export type WeekBriefResult =
  | { ok: true; days: WeekBriefDayResult[] }
  | { ok: false; error: string }

// Полный прогон: контекст проекта → промпт → Claude → {day, brief}[].
// `reader` — клиент для чтения данных проекта (session в роуте, admin в джобе:
// владение проверено при создании джоба). Все ошибки санитизируются здесь же
// (job.error и ответ роута читает клиент — граница доверия).
export async function generateWeekBrief(
  reader: SupabaseClient,
  projectId: string,
  days: BriefDay[],
): Promise<WeekBriefResult> {
...
ВАЖНО: для каждого дня в items добавь по одному элементу на КАЖДЫЙ формат из строки «форматы:» этого дня (и только на них). Темы внутри одного дня — про РАЗНЫЕ углы общей темы дня.`
  ...
  try {
    ...
    return { ok: true, days: briefDays }
  } catch (err) {
    // Сырец — в телеметрию; наружу (ответ роута / job.error) — готовый текст,
    // как и раньше отдавал роут (граница доверия, error-text-hygiene).
    console.error('[generate-week-brief] error:', err instanceof Error ? err.message : err)
    await captureException(err, { where: 'generate-week-brief', projectId })
    return { ok: false, error: AI_BUSY_MESSAGE }
  }
}

// Форматы дня, как их видит и клиент (дефолт пост/сторис/рилз).
export function weekBriefDayFormats(d: BriefDay): string[] {
  return (d.formats && d.formats.length > 0) ? d.formats : ['post', 'stories', 'reels']
}
