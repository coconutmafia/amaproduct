// Ядро «Плана прогрева»: загрузка контекста проекта, промпт, форс-тул и один
// прогон Claude. Вынесено из app/api/ai/warmup-plan 24.08, чтобы фоновый джоб
// (lib/jobs/runWarmupPlanJob.ts) использовал РОВНО ту же логику, что и
// SSE-роут — промпт и формат не раздваиваются (паттерн lib/research/table1.ts).
...
export type WarmupPlanResult =
  | { ok: true; planData: Record<string, unknown> }
  | { ok: false; error: string }

// Полный прогон: контекст проекта → промпт → Claude (стрим) → валидация.
// `reader` — клиент для чтения данных проекта (session в SSE-роуте, admin в
// джобе: владение проверено при создании джоба). Методология читается
// service-role клиентом всегда (миграция 036 закрыла её от authenticated).
// onProgress дёргается на каждый чанк стрима (heartbeat для SSE/джоба).
export async function generateWarmupPlan(
  reader: SupabaseClient,
  input: WarmupPlanInput,
  onProgress?: () => void,
): Promise<WarmupPlanResult> {
  const {
  ...
  const { data: project } = await reader
    ...
    .single()
  if (!project) {
    console.error(`[warmup-plan] Project not found: id=${projectId}`)
    return { ok: false, error: 'Проект не найден — обнови страницу и попробуй ещё раз.' }
  }
  console.log(`[warmup-plan] Starting for project="${project.name}" duration=${duration}`)
  ...
  const { data: chunks } = await reader
  ...
  if (chunks && chunks.length > 0) {
    ...
    const { data: rawMaterials } = await reader
  ...
  try {
    const { data: blogMaterials } = await reader
  ...
  const { data: materials } = await reader
  ...
  let competitorsText = ''
  try {
    const { data: igMats } = await reader
      .from('project_materials')
  ...
  try {
    ...
    const [{ data: sysTrends }, { data: projTrends }, { data: sysReels }, { data: projReels }] = await Promise.all([
      reader.from('content_trends').select('title, description, niches')
      ...
      reader.from('content_trends').select('title, description, niches')
        ...
        .order('created_at', { ascending: false }).limit(10),
      reader.from('viral_reels').select('reel_type, analysis, niches')
...
Заполни все дни от 1 до ${duration}.`
  ...
  try {
    ...
    // Каждый чанк — heartbeat наружу (SSE-прогресс или бамп job.progress)
    for await (const chunk of claudeStream) {
      if (chunk.type === 'content_block_delta') onProgress?.()
    }
    ...
    if (!toolBlock || toolBlock.type !== 'tool_use') {
      ...
      return { ok: false, error: 'AI не вернул план. Попробуй ещё раз.' }
    }
    const planJson = JSON.stringify(toolBlock.input)
    const input2 = toolBlock.input as { strategy_summary?: string; phases?: unknown }
    ...
    if (phases.length === 0) {
      console.error(`[warmup-plan] Incomplete plan (stop_reason=${finalMessage.stop_reason}), phases=${JSON.stringify(input2.phases)}, size=${planJson.length}`)
      return { ok: false, error: 'AI не успел сформировать полный план. Попробуй ещё раз.' }
    }
    const planData = { ...input2, phases }
    console.log(`[warmup-plan] Done, phases=${phases.length}, plan size=${planJson.length} bytes, stop_reason=${finalMessage.stop_reason}`)
    return { ok: true, planData }
  } catch (err) {
    // Сырец — в телеметрию; наружу ТОЛЬКО готовый русский текст. Раньше SSE-роут
    // отдавал err.message как есть — сюда попадали хвосты провайдера
    // («credit balance…») — граница доверия (см. error-text-hygiene).
    console.error('[warmup-plan] Stream error:', err instanceof Error ? err.message : err)
    await captureException(err, { where: 'generateWarmupPlan', projectId })
    return { ok: false, error: 'Генерация сейчас перегружена или временно недоступна — попробуй через минуту-две. Введённые данные не потерялись.' }
  }
}
