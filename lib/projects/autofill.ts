// Ядро «Автозаполнения мастера проектов»: скрейп Telegram/Instagram + разбор
...
export function extractUsername(url: string): { username: string; platform: string } {
...
export type AutofillOutcome =
  | { ok: true; platform: string; niche: string; description: string; target_audience: string; content_goals: string }
  | { ok: false; status: 422 | 500; error: string }

// Полный прогон: скрейп (Telegram надёжнее — первым, Instagram фолбэком) →
// разбор профиля Claude → поля проекта. Все тексты ошибок — готовые для
// клиента (job.error и ответ роута — граница доверия).
export async function runAutofill({ instagramRaw, telegramRaw }: AutofillInput): Promise<AutofillOutcome> {
  let bio = ''
  let posts: string[] = []
  let platformLabel = ''
  ...
  if (!platformLabel && instagramRaw) {
    try {
      ...
      // Telegram scrape already consumed part of the function budget.
