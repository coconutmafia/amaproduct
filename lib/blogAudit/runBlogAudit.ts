...
0 — нет совсем, 1 — есть частично/слабо, 2 — сделано хорошо.

КРИТИЧЕСКИ ВАЖНО — не утверждай факты, которых не видел. У тебя НЕТ данных о фактических продажах,
доходе, числе заявок, охватах, реальных результатах клиентов. НЕ пиши «продажи низкие/проседают»,
«мало заявок», «плохой доход» и подобное — ты этого не знаешь. Говори только про то, что видно в
профиле: насколько блог УПАКОВАН и ВЫСТРОЕН на продажу (структура, смыслы, воронка, доверие, ясность).
Формулируй как «блог/структура не выстроены на продажу», а не как «продажи низкие». Все выводы — строго
на основе полученных данных аккаунта.
...
ВАЖНО — не выдумывай пробелы, которых нет. «Жирная строка» профиля — это ИМЯ аккаунта (fullName); в
данных оно в СКОБКАХ после «@ник» в строке «Аккаунт: @… (…)», а не первая строка Bio. Ключевые слова
(ниша / город / услуга) часто именно там — проверяй и имя, и Bio.
Локация может быть на любом языке и в любом виде: город, страна, регион, курорт или «онлайн»/«по всему
миру» (напр. «Punta Cana», «Москва», «online»). Если география указана где угодно в шапке — НЕ пиши, что
города нет, и не снижай за это балл.
Внимательно перечитай имя, Bio и посты ПЕРЕД тем, как отметить что-то отсутствующим: если город/локация,
...
Пункт «Есть ли вовлечение: вопросы, реакции, голосования, обсуждения?» (блок "warmup" / Прогрев) оценивай по
АКТИВНОСТИ В КОММЕНТАРИЯХ к постам, а НЕ по тексту подписей. У каждого поста указано число комментариев
(💬 N) и лайков (❤ N). Живое обсуждение (заметное число комментариев относительно охвата/числа подписчиков)
→ вовлечение есть (1–2). Комментарии почти отсутствуют → вовлечения нет (0). Не ищи слова «вопрос/опрос» в
подписях — смотри на реальные 💬.
...
- note — ОТВЕТ НА ВОПРОС пункта, 1 короткая фраза по-русски, обращайся на «ты».
  Каждый пункт сформулирован ВОПРОСОМ, поэтому note ОБЯЗАН начинаться с «Да», «Частично» или «Нет»,
  затем — тире и что именно есть / чего не хватает. Так человек читает «вопрос → ответ», а не гадает.
  Примеры: "Да — ЦА понятна: девушки, танцы, Новосибирск"; "Частично — боли названы, но намёками";
  "Нет — результат ученика не описан". Слово выбирай по score: 2 → «Да», 1 → «Частично», 0 → «Нет».
...
- "summary": 1–2 предложения — общий честный вердикт про УПАКОВКУ и структуру блога, БЕЗ домыслов о
  фактических продажах/доходе/охватах (этих данных у тебя нет).
  ...
  const blocks: AuditBlockResult[] = CHECKLIST.map(block => {
    ...
    const items: AuditItemResult[] = block.items.map((item, i) => {
      ...
      if (!itemVisible(block.key, item.fromText, hasImages)) {
        ...
        return { label: item.label, assessable: false, score: null, note: 'Не видно из профиля — разберём на консультации' }
      }
      ...
      if (score === null) {
        ...
        scored += 0
        assessableMax += 2
        return { label: item.label, assessable: true, score: 0, note: note || 'Нет — не нашли в шапке и постах' }
      }
      scored += score
      assessableMax += 2
