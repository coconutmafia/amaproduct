...
describe('провайдерские/денежные хвосты не доходят до клиента', () => {
  ...
  // Скрин клиента 11 августа: Safari рвёт заливку по мобильной сети и отдаёт
  // «Load failed» — русский префикс протаскивал сырец на экран.
  it('прячет сафариевский «Load failed» даже за русским префиксом', () => {
    const out = friendlyError('Ошибка загрузки: Load failed', 'Проблема со связью.')
    expect(out).toBe('Проблема со связью.')
  })
