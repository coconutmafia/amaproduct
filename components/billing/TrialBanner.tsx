...
export function TrialBanner({ status, trialEndsAt }: { status?: string | null; trialEndsAt?: string | null }) {
  ...
  // Под моделью «плати сразу» у нового юзера триала нет вообще (trial_ends_at =
  // момент регистрации), поэтому формулировка «пробный период закончился» его бы
  // только путала — текст нейтральный и работает в обоих случаях.
  const text =
    kind === 'soon'      ? `Пробный период заканчивается через ${daysLeft} ${plural(daysLeft)}. Выбери тариф, чтобы не потерять доступ.`
    : kind === 'view_only' ? 'Чтобы создавать контент, подключи тариф — все твои данные на месте.'
