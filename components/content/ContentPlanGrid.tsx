                    ...
                    <div className="space-y-2.5">
                      {(() => {
                        ...
                        return entries.map(type => {
                          ...
                          // Types that have a visual editor (post image / stories / carousel).
                          const hasEditor = type === 'post' || type === 'stories' || type === 'carousel'
                          return (
                            <div key={type} className="rounded-lg border border-[#F0F0F0] p-2.5 space-y-2">
                              <div className="flex items-center justify-between gap-2">
                                <div className="flex items-center gap-1.5 min-w-0">
                                  <span className="inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-md shrink-0"
                                    style={{ backgroundColor: c.bg, color: c.text, border: `1px solid ${c.border}` }}>
                                    {c.label.toUpperCase()}
                                  </span>
                                  {existing ? (
                                    <span className="inline-flex items-center gap-0.5 text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-green-50 text-green-700 border border-green-200 shrink-0">
                                      <Check className="h-2.5 w-2.5" /> Готово
                                    </span>
                                  ) : (
                                    <span className="text-[9px] font-medium text-[#bbb] shrink-0">нужно создать</span>
                                  )}
                                </div>
                                {onRemoveType && (
                                  <button type="button" onClick={() => onRemoveType(day.day, type)} aria-label="Убрать формат"
