...
// Августе (04.09, замечание Матвея): вопросы только про ЕЁ проект — чужой
// контекст (Станислав) она оценить не может. Станислав остаётся для судьи
// и экономики (самый тяжёлый контекст в проде).
...
for (const proj of [projStan, projAvg]) {
  ...
  const qs = proj.id === projAvg.id ? AUGUSTA_QUESTIONS : QUESTIONS
  for (const q of qs) {
    pairNo++
